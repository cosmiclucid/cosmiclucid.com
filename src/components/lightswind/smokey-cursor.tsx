...
export default function SmokeyCursor({
  ...
  const baseCanvasSize = 320;
  const canvasSizeRef = useRef(baseCanvasSize);
  const halfCanvasRef = useRef(baseCanvasSize / 2);

  useEffect(() => {
    const canvas = canvasRef.current;
    const container = containerRef.current;
    if (!canvas || !container) return; // Guard early

    const computeCanvasSize = () => {
      const width =
        typeof window !== 'undefined' ? window.innerWidth : baseCanvasSize;
      const height =
        typeof window !== 'undefined' ? window.innerHeight : baseCanvasSize;
      const minDim = Math.min(width, height);
      const scale = Math.min(Math.max(intensity, 0.3), 1.5);
      const target = Math.max(200, Math.min(minDim, baseCanvasSize * scale));
      canvasSizeRef.current = target;
      halfCanvasRef.current = target / 2;
    ...
    const moveContainer = (clientX: number, clientY: number) => {
      const halfCanvas = halfCanvasRef.current;
      const width =
        typeof window !== 'undefined' ? window.innerWidth : baseCanvasSize;
      const height =
        typeof window !== 'undefined' ? window.innerHeight : baseCanvasSize;
      const clampedX = clampValue(clientX, halfCanvas, width - halfCanvas);
      const clampedY = clampValue(clientY, halfCanvas, height - halfCanvas);
      centerRef.current = { x: clampedX, y: clampedY };
      container.style.transform = `translate(${clampedX - halfCanvas}px, ${clampedY - halfCanvas}px)`;
    };
    ...
    const getLocalCoords = (clientX: number, clientY: number) => {
      ...
      const localX = clientX - (cx - halfCanvas);
      const localY = clientY - (cy - halfCanvas);
      const clampedX = clampValue(localX, 0, canvasSize);
      const clampedY = clampValue(localY, 0, canvasSize);
      const texcoordX = clampedX / canvasSize;
      const texcoordY = 1 - clampedY / canvasSize;
      return { texcoordX, texcoordY };
    };

    const halfCanvas = halfCanvasRef.current;
    const initialX = typeof window !== 'undefined' ? window.innerWidth / 2 : halfCanvas;
    const initialY = typeof window !== 'undefined' ? window.innerHeight / 2 : halfCanvas;
    moveContainer(initialX, initialY);
    ...
    function updatePointerDownData(
      ...
      pointer.down = true;
      pointer.moved = false;
      const { texcoordX, texcoordY } = getLocalCoords(clientX, clientY);
      ...
      pointer.localX = texcoordX;
      pointer.localY = texcoordY;
      pointer.texcoordX = 0.5;
      pointer.texcoordY = 0.5;
      pointer.prevTexcoordX = 0.5;
      pointer.prevTexcoordY = 0.5;
    }
    ...
    function updatePointerMoveData(
      ...
      const { texcoordX, texcoordY } = getLocalCoords(clientX, clientY);
      const deltaNormX = texcoordX - pointer.localX;
      const deltaNormY = texcoordY - pointer.localY;
      pointer.localX = texcoordX;
      pointer.localY = texcoordY;
      pointer.deltaX = correctDeltaX(deltaNormX)!;
      pointer.deltaY = correctDeltaY(deltaNormY)!;
      pointer.texcoordX = 0.5;
      pointer.texcoordY = 0.5;
      pointer.prevTexcoordX = 0.5;
      pointer.prevTexcoordY = 0.5;
    ...
    const handleMouseDown = (e: MouseEvent) => {
      ...
      if (followMouse) moveContainer(e.clientX, e.clientY);
    ...
    function handleFirstMouseMove(e: MouseEvent) {
      ...
      if (followMouse) {
        moveContainer(e.clientX, e.clientY);
        pointer.localX = 0.5;
        pointer.localY = 0.5;
      }
    ...
    function handleFirstTouchStart(e: TouchEvent) {
      ...
      for (let i = 0; i < touches.length; i++) {
        updateFrame();
        if (followMouse) moveContainer(touches[i].clientX, touches[i].clientY);
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      for (let i = 0; i < touches.length; i++) {
        ...
        if (followMouse) {
          moveContainer(touches[i].clientX, touches[i].clientY);
          pointer.localX = 0.5;
          pointer.localY = 0.5;
        }
      }
  ...
  return (
    <div
      ref={containerRef}
      className={`fixed pointer-events-none cursor-none ${className}`}
      style={{ width: canvasSizeRef.current, height: canvasSizeRef.current, zIndex: -5, left: 0, top: 0 }}
    >
      <canvas
        ref={canvasRef}
        id="fluid"
