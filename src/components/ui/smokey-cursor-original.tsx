...
const COSMIC_CORE_TINT: ColorRGB = { r: 0.6, g: 0.32, b: 1.05 };

function toneBurstColor(color: ColorRGB) {
  const blend = 0.34;
  let r = color.r * (1 - blend) + COSMIC_CORE_TINT.r * blend;
  let g = color.g * (1 - blend) + COSMIC_CORE_TINT.g * blend;
  let b = color.b * (1 - blend) + COSMIC_CORE_TINT.b * blend;

  const softCap = 1.08;
  const maxBeforeCap = Math.max(r, g, b);
  if (maxBeforeCap > softCap) {
    const scale = softCap / maxBeforeCap;
    r *= scale;
    g *= scale;
    b *= scale;
  }

  const maxAfterCap = Math.max(r, g, b);
  const minAfterCap = Math.min(r, g, b);
  if (maxAfterCap - minAfterCap < 0.18) {
    const saturationBoost = 0.28;
    r += (COSMIC_CORE_TINT.r - r) * saturationBoost;
    g += (COSMIC_CORE_TINT.g - g) * saturationBoost;
    b += (COSMIC_CORE_TINT.b - b) * saturationBoost;
  }
...
export default function SmokeyCursor({
  ...
  useEffect(() => {
    ...
    function scaleByPixelRatio(input: number, floorValue = true) {
      const pixelRatio = window.devicePixelRatio || 1;
      const value = input * pixelRatio;
      return floorValue ? Math.floor(value) : value;
    }
    ...
    function clickSplat(pointer: Pointer) {
      ...
      const burstScale = 1.6;
      color.r *= burstScale;
      color.g *= burstScale;
      color.b *= burstScale;
      toneBurstColor(color);
      const dx = 6 * (Math.random() - 0.5);
      const dy = 14 * (Math.random() - 0.5);
      splat(pointer.texcoordX, pointer.texcoordY, dx, dy, color, 0.35);
    }
    ...
    function generateColor(): ColorRGB {
      const palette: Array<{ color: ColorRGB; weight: number }> = [
        { color: { r: 0.35, g: 0.55, b: 1.0 }, weight: 5 }, // luminous blue (dominant)
        { color: { r: 0.75, g: 0.35, b: 0.98 }, weight: 4 }, // cosmic purple (dominant)
        { color: { r: 0.2, g: 0.9, b: 0.95 }, weight: 2 }, // electric cyan highlight
        { color: { r: 1.0, g: 0.45, b: 0.7 }, weight: 1 }, // nebula pink highlight
        { color: { r: 1.0, g: 0.82, b: 0.4 }, weight: 1 }, // warm gold highlight
        { color: { r: 0.58, g: 0.52, b: 0.92 }, weight: 1 }, // violet aurora highlight (dimmed for transparency)
      ];
