...
interface SparkleNavbarProps {
   ...
   * Provides the item index and label.
   */
  onItemClick?: (index: number, label: string) => void;
  /**
   * Optional label that should appear active (used to sync with router).
   */
...
const SparkleNavbar: React.FC<SparkleNavbarProps> = ({
  ...
  useLayoutEffect(() => {
    const activeButton =
      activeIndex !== null ? buttonRefs.current[activeIndex] : null;
    if (navRef.current && activeElementRef.current && activeButton) {
      ...
      gsap.set(activeElementRef.current, { "--active-element-show": "0" });
    }
  }, [activeIndex]);

  useEffect(() => {
    if (activeLabel) {
      const index = items.findIndex((label) => label === activeLabel);
  ...
  const handleClick = (index: number) => {
    ...
    const oldButton = activeIndex !== null ? buttonRefs.current[activeIndex] : null;
    ...
    if (activeIndex === null || !oldButton || index === activeIndex) {
    ...
    const direction = index > (activeIndex ?? -1) ? "after" : "before";
    ...
    gsap.to(activeElement, {
      keyframes: [
        {
          "--active-element-width": `${Math.min(navElement.offsetWidth - 20, spacing + 40)}px`,
          duration: 0.3,
          ease: "none",
          onStart: () => {
